...
<html lang="en">
    ...
    <body>
        <header>
            ...
            <h2 class="list_h2">영화 리뷰 게시판</h2>
            <h3 class="list_h3">게시글 목록</h3>
               <form action="<c:url value="/board/reviewlist.do" />" method="get" id="form">
                <div class="searchDiv">
           ...
           <c:choose>
            ...
            <div>
				...
				<c:if test="${currentPage > 1}">
						<li><a href="<c:url value="/board/reviewlist.do?page=1" />">&lt;&lt;</a></li>
						<li><a href="<c:url value="/board/reviewlist.do?page=${currentPage-1}" />">&lt;</a></li>
				</c:if>
				<c:forEach begin="${startPage}" end="${endPage > totalPage ? totalPage : endPage}" var="pageNum">
					<c:choose>
						...
						<c:otherwise>
							<li><a href="<c:url value="/board/reviewlist.do?page=${pageNum}" />">${pageNum}</a></li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				<c:if test="${currentPage < totalPage}">
					<li><a href="<c:url value="/board/reviewlist.do?page=${currentPage+1}" />">&gt;</a></li>
					<li><a href="<c:url value="/board/reviewlist.do?page=${totalPage}" />">&gt;&gt;</a></li>
				</c:if>
				</ul>
            </div>
    </body>
