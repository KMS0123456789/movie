        ...
        <header>
            ...
            <div>
				...
				<c:if test="${currentPage > 1}">
						<a href="<c:url value="/board/reviewlist.do?page=1" />">처음 페이지</a>
						<a href="<c:url value="/board/reviewlist.do?page=${currentPage-1}" />">이전 페이지</a>
				</c:if>
				<c:forEach begin="${startPage}" end="${endPage > totalPage ? totalPage : endPage}" var="pageNum">
					<c:choose>
						...
						<c:otherwise>
							<a href="<c:url value="/board/reviewlist.do?page=${pageNum}" />">${pageNum}</a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				<c:if test="${currentPage < totalPage}">
					<a href="<c:url value="/board/reviewlist.do?page=${currentPage+1}" />">다음 페이지</a>
					<a href="<c:url value="/board/reviewlist.do?page=${totalPage}" />">마지막 페이지</a>
				</c:if>
            </div>
            <br>
